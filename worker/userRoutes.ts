...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.get('/api/graph', async (c) => {
        try {
            ...
            return c.json({ success: false, error: 'Failed to retrieve graph data' }, { status: 500 });
        }
    });
    app.post('/api/graph', async (c) => {
        try {
            ...
            const controller = getAppController(c.env);
            await controller.updateGraph(graphData);
            return c.json({ success: true, data: { updated: true } });
        } catch (error) {
            console.error('Failed to update graph:', error);
            return c.json({ success: false, error: 'Failed to update graph data' }, { status: 500 });
        }
    });
    app.post('/api/graph/reset', async (c) => {
        try {
            const controller = getAppController(c.env);
            const newGraph = await controller.resetGraph();
            return c.json({ success: true, data: newGraph });
        } catch (error) {
            console.error('Failed to reset graph:', error);
            return c.json({ success: false, error: 'Failed to reset graph data' }, { status: 500 });
        }
    });
    // --- Session Routes ---
